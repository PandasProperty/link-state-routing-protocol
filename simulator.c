			...
			if (FD_ISSET(i, &tmp_fds)) {
				...
				case 5: {
					...
					if (mes_fin == last_topologie+1) { //daca toate nodurile din topologie au finalizat procesarea mesajelor de la timpul curent
						//se trimit evenimente numai pe pasi pari
						if (((timp+1)%2==0) && (timp+1 < 2*nr_ev)) {  //daca mai am evenimente, le anunt nodurilor direct implicate
							mesaj.type = 7; //anunt eveniment
							sprintf (mesaj.payload, "%s\n", events[(timp+1)/2].even);	
							
							nod1 = events[(timp+1)/2].d1;
							mesaj.add = FALSE; //adevarat doar pentru eveniment tip 1
							
							if (events[(timp+1)/2].type == 1) { //adaugare nod la topologie
								...
								if (fdmax < pipec[nod1][0])
									fdmax = pipec[nod1][0];
								mesaj.add = TRUE;	
								mesaj.time = timp;  //instiintez noul nod de pasul de timp al simularii
							}
							
							out = pipec[nod1][3];
							write(out, &mesaj, sizeof(msg));
						
							nod2 = events[(timp+1)/2].d2;
							if (nod2 > -1){
								out = pipec[nod2][3];
								write(out, &mesaj, sizeof(msg));  //mesaj 7 anunt eveniment
							
							}
						}

						if ((nr_msg == 0) && (timp >= 2*nr_ev)) {//daca nu s-au primit mesaje (din protocolul linkstate) in aceasta runda si nu mai sunt evenimente
						...
						for (nod = 0; nod <= nodmax; nod++) {
							out = pipec[nod][3];
							mesaj2.type = 8; 
							write(out, &mesaj2, sizeof(msg));  //mesaj 8 cer tabela de rutare
						}	
						
						if (mesaj.type == 6)
							for (nod = 0; nod <= last_topologie; nod++) {
								out = pipec[nod][3];
								write(out, &mesaj, sizeof(msg));  //se da liber la procesare mesaje vechi (primite la t-1) din coada
							}	
						
					if (mesaj.type == 9) {
							
							for (nod = 0; nod < KIDS; nod++) {
								out = pipec[nod][3];
								write(out, &mesaj, sizeof(msg));  //s-a terminat simularea
							}	
